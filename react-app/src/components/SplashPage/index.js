      ...
      <div className='splash-page-wrapper'>
        ...
        <div className='splash-page-balance-container'>
          ...
          <div className='splash-page-owe-container'>
            ...
            <div>{totalOwe.toFixed(2)}</div>
          </div>
          <div className='splash-page-are-owe-container'>
            <div>
              <h3>You are owed</h3>
            </div>
            <div>{totalOwed.toFixed(2)}</div>
          </div>
        </div>
        <div className='splash-page-content-container'>
          <div className='you-owe-details-container'>
            ...
            {expensesArr
              .filter(expense => expense.payer.id !== sessionUser.id)
              .map(expense => {
                const owedAmount = expense.amount / (expense.owers.length + 1)
                return (
                  <div key={expense.id} className='you-owe-row'>
                    <div className='you-owe-payer'>
                      {expense.payer.firstName}
                    </div>
                    <div className='you-owe-amount'>
                      {' '}
          ...
          <div className='you-are-owed-details-container'>
            ...
            {expensesArr
              ...
              .map(expense => {
                ...
                return (
                  <div key={expense.id} className='are-owed-you-row'>
                    {expense.owers.map(ower => (
                      <div key={ower.id} className='are-owed-you-ower'>
                        {/* <div>{ower.firstName}</div> */}
                        <div>{ower.firstName} owes you ${amountOwed.toFixed(2)}</div>
                      </div>
                    ))}
                  </div>
                )
